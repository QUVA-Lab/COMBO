...
from GraphDecompositionBO.graphGP.sampler.tool_partition import group_input, ungroup_input


def neighbors(x, partition_samples, edge_mat_samples, n_vertex, uniquely=False):
    """
    ...
    for i in range(len(partition_samples)):
        grouped_x = group_input(x, partition_samples[i], n_vertex)
        grouped_nbd = _cartesian_neighbors(grouped_x, edge_mat_samples[i])
        nbd = ungroup_input(grouped_nbd, partition_samples[i], n_vertex)
        added_ind = []
        if uniquely:
            for j in range(nbd.size(0)):
                if not torch.any(torch.all(nbds, nbd[j])):
                    added_ind.append(j)
            if len(added_ind) > 0:
...
def _cartesian_neighbors(grouped_x, edge_mat_list):
    ...
    for i in range(len(edge_mat_list)):
        nbd_i_elm = edge_mat_list[i][grouped_x[i]].nonzero()
