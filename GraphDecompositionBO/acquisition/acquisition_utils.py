...
def acquisition_expectation(x, inference_samples, acquisition_func=expected_improvement, reference=None):
    ...
    for s in range(len(inference_samples)):
        pred_dist = inference_samples[s].predict(x, verbose=False)
...
def inference_sampling(input_data, output_data, n_vertex, hyper_samples, partition_samples, freq_samples, basis_samples):
    """
    ...
    """
    inference_samples = []
    for s in range(hyper_samples.size(0)):
        ...
        grouped_input_data = group_input(input_data=input_data, sorted_partition=partition_samples[s], n_vertex=n_vertex)
        inference = Inference((grouped_input_data, output_data), model=model)
        inference_samples.append(inference)
    return inference_samples


def suggestion_statistic(x, inference_samples, partition_samples, n_vertex):
    if x.dim() == 1:
        x = x.unsqueeze(0)
    mean_sample_list = []
    std_sample_list = []
    var_sample_list = []
    for s in range(len(inference_samples)):
        grouped_x = group_input(input_data=x, sorted_partition=partition_samples[s], n_vertex=n_vertex)
