...
class Branin(object):
	...
	def __init__(self):
		self.n_vertices = [51] * 2
		self.n_factors = len(self.n_vertices)
		self.suggested_init = torch.Tensor(self.n_vertices).long().unsqueeze(0) / 2
		for _ in range(1, 2):
			self.suggested_init = torch.cat([self.suggested_init, torch.cat([torch.randint(0, elm, (1, 1)) for elm in self.n_vertices], dim=1)], dim=0)
		self.adjacency_mat = []
		self.fourier_coef = []
		self.fourier_basis = []
	...
	def evaluate(self, x_g):
		...
		for d in range(len(self.n_vertices)):
			x_e[:, d] = torch.linspace(-1, 1, self.n_vertices[d])[x_g[:, d].long()]
		...
		for i in range(n_repeat):
			output += a * (x_e[:, 2 * i + 1] - b * x_e[:, 2 * i] ** 2 + c * x_e[:, 2 * i] - r) ** 2 + s * (1 - t) * torch.cos(x_e[:, 2 * i]) + s
...
class Hartmann6(object):
	...
	def evaluate(self, x_g):
		...
		x_e = torch.ones(x_g.size())
		for d in range(len(self.n_vertices)):
			x_e[:, d] = torch.linspace(-1, 1, self.n_vertices[d])[x_g[:, d]]
